...
abstract class BasesfGuardUserFormFilter extends BaseFormFilterDoctrine
{
  ...
  public function addEstadosListColumnQuery(Doctrine_Query $query, $field, $values)
  {
    if (!is_array($values))
    {
      $values = array($values);
    ...
    $query
      ->leftJoin($query->getRootAlias().'.UserEstado UserEstado')
      ->andWhereIn('UserEstado.estado_id', $values)
    ;
  }
